...
# Only the remaining ones - simpler action descriptions to avoid timeouts
ENEMIES = [
    ("void_weaver", "dark spell attack"),
...
for idx, (name, action) in enumerate(ENEMIES, 1):
    ...
    if not os.path.exists(sprite_path):
        print(f"  ERROR: Sprite not found", flush=True)
        fail += 1
        continue
...
print(f"\nDone: {success}/{total} succeeded, {fail} failed", flush=True)
